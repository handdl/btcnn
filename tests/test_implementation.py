...
def test_layer_norm_implementation(
    ...
    binary_tree_layer_norm = BinaryTreeLayerNorm(in_channels=2, eps=0, frozen=True)
    with torch.no_grad():
...
def test_instance_norm_implementation(
    ...
    binary_tree_instance_norm = BinaryTreeInstanceNorm(in_channels=2, eps=0, frozen=True)
    with torch.no_grad():
...
def test_full_block_implementation(
    ...
    block = BinaryTreeSequential(
        ...
        BinaryTreeLayerNorm(in_channels=1, eps=0, frozen=True),
